...
> `JSR-330 Provider`
> 
> 자바 표준을 사용하는 방법, 별도의 라이브러리를 필요로하며 스프링에 의존❌

<br>

#### ❔ 프로토타입 빈을 사용하는 경우
매번 사용할 때 마다 의존관계 주입이 완료된 새로운 객체가 필요할 경우 사용 (대부분 싱글톤으로 해결가능)

<br>

### 1-5. 웹 스코프

* 웹 환경에서만 동작하고 프로토타입과 다르게 스프링이 해당 스코프의 종료시점까지 관리 (종료메서드 호출)

<br>

#### 웹 스코프 종류

* `request` : HTTP 요청 하나가 들어오고 나갈 때 까지 유지되는 스코프, 각각의 요청마다 별도로 인스턴스 생성/관리
